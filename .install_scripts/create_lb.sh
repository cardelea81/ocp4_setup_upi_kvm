...
echo -n "====> Downloading Centos 9 cloud image: "; download get "$LB_IMG" "$LB_IMG_URL";

echo -n "====> Copying Image for Loadbalancer VM: "
cp "${CACHE_DIR}/CentOS-Stream-GenericCloud-9-20240527.0.x86_64.qcow2" "${VM_DIR}/${CLUSTER_NAME}-lb.qcow2" || \
    err "Copying '${VM_DIR}/CentOS-Stream-GenericCloud-9-20240527.0.x86_64.qcow2' to '${VM_DIR}/${CLUSTER_NAME}-lb.qcow2' failed"; ok
...
virt-install --import --name ${CLUSTER_NAME}-lb --disk "${VM_DIR}/${CLUSTER_NAME}-lb.qcow2" \
    --memory ${LB_MEM} --cpu host --vcpus ${LB_CPU} --os-type linux --os-variant rhel9.0 --network network=${VIR_NET},model=virtio \
